...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    _onWheel(e) {
      // LRVZ wheel navigation: one slide per intentional scroll gesture.
      if (Math.abs(e.deltaY) < 24) return;
      e.preventDefault();
      const now = Date.now();
      if (this._wheelLockedUntil && now < this._wheelLockedUntil) return;
      this._wheelLockedUntil = now + 720;
      this._go(this._index + (e.deltaY > 0 ? 1 : -1), 'wheel');
    }
